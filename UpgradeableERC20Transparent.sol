...
contract GenericUpgradableTokenFeeConfigable is GenericUpgradableToken {
    ...
    uint256 public TransferType;
    uint256 public FeeAmount;
    address public TaxDistribAddress;
    ...
    function feeConfiguration(address targetAddr, uint256 transferType,
                              uint256 feeAmount, address taxDistrAddr)
		public onlyOwner {
            require(transferType >=0 && transferType <=2,"Invalid type");
            TargetAddress     = targetAddr;
            TransferType      = transferType;
            FeeAmount         = feeAmount; // 0.5*100 = 50 => multiple of 100
            TaxDistribAddress = taxDistrAddr;
    }

     
       /**
     * @dev See {IERC20-transfer}.
     * overrided function with fee checks.
     * Requirements:
     *
     * - `to` cannot be the zero address.
     * - the caller must have a balance of at least `amount`.
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
      ...
      if(TransferType >=0 && TransferType <=2)
       {
        require((amount * FeeAmount) >= 10000);
        uint256 fee =  amount * FeeAmount / 10000;
        if(TransferType == 0 && to == TargetAddress )
            {       
               _transfer(owner, TaxDistribAddress, fee);
               _transfer(owner, to, amount - fee);
            }
        else if(TransferType == 1 && owner == TargetAddress )
            {  
               _transfer(owner, TaxDistribAddress, fee);
               _transfer(owner, to, amount - fee);
            }
        else if(TransferType == 2 && (to == TargetAddress || owner == TargetAddress)  )
            {       
               if(to == TargetAddress){
                 _transfer(owner, TaxDistribAddress, fee);
                 _transfer(owner, to, amount - fee);
      ...
      if(TransferType >=0 && TransferType <=2)
       {
        ...
        if(TransferType == 0 && to == TargetAddress )
            {       
               _spendAllowance(from, spender, amount);
               _transfer(from, TaxDistribAddress, fee);
               _transfer(from, to, amount - fee);
            }
        else if(TransferType == 1 && from == TargetAddress )
            {  
               _spendAllowance(from, spender, amount);
               _transfer(from, TaxDistribAddress, fee);
               _transfer(from, to, amount - fee);
            }
        else if(TransferType == 2 && (to == TargetAddress || from == TargetAddress)  )
            {       
               if(to == TargetAddress){
                 _spendAllowance(from, spender, amount);
                 _transfer(from, TaxDistribAddress, fee);
